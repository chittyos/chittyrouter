...
vi.mock('../../src/utils/chittyid-generator.js', () => ({
  generateEmailChittyID: vi.fn().mockResolvedValue('CHITTY-TEST-456')
}));
...
describe('EmailProcessor', () => {
  ...
  describe('processIncomingEmail', () => {
    ...
    it('should use fallback processing when AI routing fails', async () => {
      ...
      failingProcessor.router.intelligentRoute = vi.fn().mockRejectedValue(new Error('AI routing failed'));
  ...
  describe('extractEmailData', () => {
    ...
    it('should handle emails with multiple attachments', async () => {
      ...
      expect(emailData.attachments).toHaveLength(testEmails.document_evidence.attachments.length);
    ...
    it('should handle emails without case patterns', async () => {
      ...
      expect(emailData.casePattern === null || emailData.casePattern?.has_case_pattern === false).toBe(true);
    });
  });
  ...
  describe('extractCasePattern', () => {
    ...
    it('should use fallback extraction when AI fails', async () => {
      ...
      const pattern = await failingProcessor.extractCasePattern(to, subject, content);
  ...
  describe('executeRoutingActions', () => {
    it('should execute all routing actions', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Routing to:'));
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('ChittyID generated:'));
    ...
    it('should handle unknown action types gracefully', async () => {
      ...
      const routingResult = {
        chittyId: 'CHITTY-123',
        actions: [
          { type: 'UNKNOWN_ACTION_TYPE', data: 'test' }
        ]
      };
  ...
  describe('sendAIResponse', () => {
    it('should send AI-generated response', async () => {
      ...
      const replySpy = vi.spyOn(message, 'reply').mockResolvedValue({ success: true });
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    it('should handle response sending failures', async () => {
      ...
      const replySpy = vi.spyOn(message, 'reply').mockRejectedValue(new Error('Send failed'));
      const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

      await processor.sendAIResponse(message, response);

      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to send AI response:', expect.any(Error));
  ...
  describe('forwardToRoutes', () => {
    it('should forward to primary route', async () => {
      ...
      const forwardSpy = vi.spyOn(message, 'forward').mockResolvedValue({ success: true });
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});

      await processor.forwardToRoutes(message, routing);

      expect(forwardSpy).toHaveBeenCalledWith('case-management@example.com');
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Forwarded to primary'));
    ...
    it('should use fallback route when forwarding fails', async () => {
      ...
      const forwardSpy = vi.spyOn(message, 'forward')
  ...
  describe('createChittyThread', () => {
    it('should create thread for case-related emails', async () => {
      ...
      expect(storeInChittyChain).toHaveBeenCalledWith(expect.objectContaining({
        chittyId: 'CHITTY-123',
    ...
    it('should handle thread creation failures', async () => {
      ...
      const message = createMockMessage('lawsuit_urgent');
      const { storeInChittyChain } = await import('../../src/utils/storage.js');
      storeInChittyChain.mockRejectedValueOnce(new Error('Thread creation failed'));

      const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

      await processor.createChittyThread(routingResult, message);

      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to create ChittyChat thread:', expect.any(Error));
  ...
  describe('escalateMessage', () => {
    ...
    it('should handle escalation failures', async () => {
      ...
      const forwardSpy = vi.spyOn(message, 'forward').mockRejectedValue(new Error('Escalation failed'));
      const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

      await processor.escalateMessage(message, action);

      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to escalate message:', expect.any(Error));
  ...
  describe('processAttachments', () => {
    it('should process multiple attachments', async () => {
      ...
      expect(attachments).toHaveLength(testEmails.document_evidence.attachments.length);
    ...
    it('should handle attachment processing failures', async () => {
      ...
      generateEmailChittyID.mockRejectedValueOnce(new Error('ChittyID generation failed'));

      const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

      const attachments = await processor.processAttachments(message);

      expect(consoleErrorSpy).toHaveBeenCalledWith('Attachment processing failed:', expect.any(Error));
      expect(attachments).toHaveLength(0);
  ...
  describe('fallbackProcessing', () => {
    ...
    it('should handle complete system failure', async () => {
      ...
      const forwardSpy = vi.spyOn(message, 'forward').mockRejectedValue(new Error('Forward failed'));
      const replySpy = vi.spyOn(message, 'reply').mockRejectedValue(new Error('Reply failed'));
  ...
  describe('parseAIResponse', () => {
    it('should parse valid JSON responses', () => {
      const response = 'Analysis: {"has_case_pattern": true, "pattern_type": "lawsuit"}';
