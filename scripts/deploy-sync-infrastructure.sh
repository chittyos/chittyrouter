...
wrangler deploy --config wrangler.do.toml -m "$ENVIRONMENT"
...
# Get worker URL
WORKER_URL=$(wrangler deployments list -m "$ENVIRONMENT" --format json | jq -r '.[0].url' 2>/dev/null || echo "")
