...
export class ChittyTelemetry {
  ...
  async initialize() {
    ...
    console.log(`Telemetry initialized for ${this.workerName} (${this.chittyId})`);
  }
  ...
  async trackRequest(request, response, duration) {
    ...
    this.incrementCounter(`http.requests.status.${response?.status || 'unknown'}`);
   ...
   * Track AI operations
   */
  async trackAIOperation(operation, model, input, output, duration, success = true) {
  ...
  isCriticalEvent(eventName) {
    return ['error.occurred', 'worker.crashed', 'ai.operation.failed'].includes(eventName);
  }

  isCriticalError(error) {
    return error.name === 'AIProcessingError' ||
