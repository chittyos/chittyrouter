...
import { validateDocumentSchema, getSchemaServiceHealth } from './schema-validation.js';
...
export async function storeInEvidenceVault(env, file, chittyId) {
  ...
  if (!schemaValidation.valid) {
    console.warn('⚠️ Document schema validation failed:', schemaValidation.errors);
  }
  ...
  if (schemaValidation.valid) {
    ...
    formData.append('complianceFlags', JSON.stringify(schemaValidation.complianceFlags));
  }
...
export async function retrieveFromChittyChain(env, identifier) {
  try {
    const endpoint = env.CHITTYOS_ENDPOINT || 'https://chittyos.com';
    const response = await fetch(`${endpoint}/api/chittychain/retrieve/${identifier}`, {
      headers: {
        'Authorization': `Bearer ${env.CHITTYCHAIN_API_KEY}`
      }
    });
...
export async function queryChittyChainByCase(env, caseId) {
  try {
    const endpoint = env.CHITTYOS_ENDPOINT || 'https://chittyos.com';
    const response = await fetch(`${endpoint}/api/chittychain/query?caseId=${caseId}`, {
      headers: {
        'Authorization': `Bearer ${env.CHITTYCHAIN_API_KEY}`
      }
    });
...
export async function generateChittyIDFromService(env, type, metadata) {
  try {
    ...
    const response = await fetch('https://id.chitty.cc/api/v1/generate', {
      ...
      headers: {
        'Content-Type': 'application/json',
        'Authorization': env.CHITTYID_API_KEY ? `Bearer ${env.CHITTYID_API_KEY}` : undefined
      },
      body: JSON.stringify({
        type: type,
        metadata: metadata
