...
export class NotionProvider {
  constructor(env) {
    ...
    this.schemaAuthority = env.CHITTYSCHEMA_ENDPOINT || 'https://schema.chitty.cc';
  }
  ...
  async storeMetadata(path, metadata) {
    try {
      ...
      const properties = {
        ...
        'Last Modified': { date: { start: metadata.lastModified || new Date().toISOString() } },
  ...
  async getTrustScore(metadata) {
    try {
      const trustAuthority = this.env.CHITTYTRUST_ENDPOINT || 'https://trust.chitty.cc';
  ...
  buildNotionProperties(path, content, options) {
    return {
      'Path': { title: [{ text: { content: path } }] },
      'Content Type': { select: { name: options.contentType || 'application/json' } },
      'Tier': { select: { name: options.tier || 'HOT' } },
      'Project': { rich_text: [{ text: { content: options.projectId || this.env.PROJECT_ID } }] },
      'Session': { rich_text: [{ text: { content: options.sessionId || this.env.SESSION_ID } }] },
      'Created': { date: { start: new Date().toISOString() } },
...
export class NeonProvider {
  ...
  async validateChittyId(chittyId) {
    if (!chittyId) {
      // Generate new ChittyID from authority
      try {
        ...
        console.warn('⚠️ ChittyID authority unavailable:', error.message);
      }

      // Fallback ID generation
      return `CHITTY-META-${Date.now()}-${Math.random().toString(36).substring(2)}`;
    }
...
export class GitHubProvider {
  constructor(env) {
    ...
    this.verifyAuthority = env.CHITTYVERIFY_ENDPOINT || 'https://verify.chitty.cc';
  }
