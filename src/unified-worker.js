...
import { ChittySchemaService } from "../../../CHITTYFOUNDATION/chittyschema/src/chittyrouter-service.js";
...
class RouteMultiplexer {
  constructor(env) {
    ...
    this.services = {
      ...
      schema: {
        schemaService: new ChittySchemaService(env),
      },
    };
    ...
    this.routes = new Map([
      ...
      // ChittySchema Routes
      ["/schema/health", this.handleSchemaHealth.bind(this)],
      ["/schema/evidence", this.handleSchemaEvidence.bind(this)],
      ["/schema/evidence/service", this.handleSchemaServiceEvidence.bind(this)],
      ["/schema/cases", this.handleSchemaCases.bind(this)],
      ["/schema/facts", this.handleSchemaFacts.bind(this)],
      ["/schema/analytics", this.handleSchemaAnalytics.bind(this)],
  ...
  // ============ ChittySchema Handlers ============

  async handleSchemaHealth(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
    );
  }

  async handleSchemaEvidence(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
  ...
  async handleSchemaServiceEvidence(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
  ...
  async handleSchemaCases(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
  ...
  async handleSchemaFacts(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
  ...
  async handleSchemaAnalytics(request) {
    const url = new URL(request.url);
    return await this.services.schema.schemaService.handleRequest(
      request,
      url.pathname,
  ...
  async handleMCP(request, url) {
    ...
    return new Response(
      JSON.stringify({
        ...
        protocol: "mcp",
        description:
          "ChittyRouter AI Gateway with ChittySchema integration and Model Context Protocol",
        tools: 28,
        categories: 17,
        endpoints: {
  ...
  async handleMCPTools(request, url) {
    ...
    const tools = {
      ...
      tools: [
        ...
        {
          name: "schema_evidence_submit",
          description: "Submit evidence to ChittySchema",
          category: "ChittySchema",
        },
        {
          name: "schema_evidence_service",
          description: "Service-orchestrated evidence ingestion",
          category: "ChittySchema",
        },
        {
          name: "schema_cases_list",
          description: "List cases in ChittySchema",
          category: "ChittySchema",
        },
        {
          name: "schema_facts_list",
          description: "List atomic facts",
          category: "ChittySchema",
        },
        {
          name: "schema_analytics",
          description: "Get ChittySchema analytics",
          category: "ChittySchema",
        },
      ],
      total: 28,
  ...
  async handleMetrics(request) {
    const metrics = {
      ...
      schema: this.services.schema.schemaService.getServiceMetadata(),
      timestamp: new Date().toISOString(),
    };
  ...
  async checkSchemaHealth() {
    try {
      // Try to call schema health endpoint internally
      const schemaService = this.services.schema.schemaService;
      const healthResponse = await schemaService.handleHealth();

      return {
        status: "healthy",
        service: "ChittySchema",
        version: schemaService.version,
