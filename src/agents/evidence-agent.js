...
export class EvidenceAgent extends ChittyRouterBaseAgent {
  ...
  async handleIngest(request) {
    ...
    const existing = this.sql.exec("SELECT id FROM evidence_items WHERE exhibit_id = ?", exhibit_id).toArray();
  ...
  async handleVerify(request) {
    ...
    const rows = this.sql.exec("SELECT * FROM evidence_items WHERE exhibit_id = ?", exhibit_id).toArray();
    ...
    if (result === "passed" && evidence.status === "ingested") {
      this.sql.exec("UPDATE evidence_items SET status = 'verified', updated_at = datetime('now') WHERE id = ?", evidence.id);
  ...
  async handleSeal(request) {
    ...
    this.sql.exec("UPDATE evidence_items SET status = 'sealed', updated_at = datetime('now') WHERE id = ?", rows[0].id);
  ...
  async handleDispute(request) {
    ...
    this.sql.exec("UPDATE evidence_items SET status = 'disputed', updated_at = datetime('now') WHERE id = ?", rows[0].id);
  ...
  handleCustody(url) {
    ...
    const evidence = this.sql.exec("SELECT * FROM evidence_items WHERE exhibit_id = ?", exhibit_id).toArray();
    if (evidence.length === 0) return this.jsonResponse({ error: "Evidence not found" }, 404);

    const chain = this.sql.exec("SELECT * FROM custody_chain WHERE evidence_id = ? ORDER BY occurred_at ASC", evidence[0].id).toArray();
    const verifications = this.sql.exec("SELECT * FROM verification_log WHERE evidence_id = ? ORDER BY verified_at ASC", evidence[0].id).toArray();
  ...
  handleStats() {
    const byCategory = this.sql.exec("SELECT category, status, COUNT(*) as count FROM evidence_items GROUP BY category, status ORDER BY count DESC").toArray();
    const total = this.sql.exec("SELECT COUNT(*) as total FROM evidence_items").toArray();
    const sealedCount = this.sql.exec("SELECT COUNT(*) as count FROM evidence_items WHERE status = 'sealed'").toArray();
    return this.jsonResponse({ totalEvidence: total[0]?.total || 0, sealedEvidence: sealedCount[0]?.count || 0, breakdown: byCategory });
  }

  handleStatus() {
    const recent = this.sql.exec("SELECT COUNT(*) as count FROM evidence_items WHERE created_at > datetime('now', '-1 hour')").toArray();
